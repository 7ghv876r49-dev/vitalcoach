...
export async function onRequest(context) {
  ...
  const key = url.searchParams.get('key');       // 'tracker', 'knowledge', 'system_prompt', or 'weeklyPlan'
  if (!key || !['tracker','knowledge','system_prompt','weeklyPlan'].includes(key)) {
    return new Response(JSON.stringify({ error: 'Invalid key. Use: tracker, knowledge, system_prompt, weeklyPlan' }), { status: 400, headers: CORS });
  }
